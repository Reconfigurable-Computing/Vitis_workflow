...
void vadd_op(dt *in1, dt *in2, dt *res1, dt *res2, int LEN, std::string xclbin){
	EventTimer et;
	...
	cl::Kernel vadd_kernel,vadd_kernel2;
	printf("load kernel start\n");
	vadd_kernel = cl::Kernel(program, "vadd_kernel:{vadd_kernel_1}", &fail);//The name "vadd_kernel" should be same as your kernel name .xo
	vadd_kernel2 = cl::Kernel(program, "vadd_kernel:{vadd_kernel_2}", &fail);
	...
	j = 0;
	vadd_kernel2.setArg(j++, In1_buf2);
	vadd_kernel2.setArg(j++, In2_buf2);
	vadd_kernel2.setArg(j++, Res_buf2);
	...
	q.enqueueTask(vadd_kernel2, &events_write, &events_kernel[1]);
